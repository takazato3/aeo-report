---
## このプロジェクトでの適用範囲
- ブログ記事の本文（H2セクション群）に適用する
- 以下は適用除外（AI引用適性を優先し、平叙で構造的に書く）：
  - 記事冒頭の結論サマリ（結論ファースト部分）
  - 末尾の「よくある質問」セクション
- 「併用する規範」（japanese-tech-writing）は本プロジェクトには
  存在しないため参照不要
- 執筆後の点検手順（話題テスト・漏出テスト等）は記事生成の
  最終ステップとして必ず実行する
- 文体はCONTENT_AUTOPILOT.mdの規定（です・ます調）に従う。
  本規範の例文の文体は模倣しない
---
